...
public class InstructorAnalyticsService {
        ...
        public InstructorAnalyticsService(CourseRepository courseRepository, EnrollmentRepository enrollmentRepository,
        ...
        public InstructorDashboardResponse getInstructorDashboard(Long instructorId) {
                ...
                int totalQuizAttempts = (int) quizAttemptRepository.findAll().stream()
                                ...
                                .filter(attempt -> instructorCourses.stream()
                                                .anyMatch(course -> course.getId().equals(
                                                                attempt.getQuiz().getLesson().getCourse().getId())))
                                .count();
        ...
        private CoursePerformanceDTO getCoursePerformance(Course course) {
                int totalEnrollments = course.getEnrolledStudents() != null ? course.getEnrolledStudents().size() : 0;

                // Calculate completed enrollments
                int completedEnrollments = 0;
                if (course.getEnrolledStudents() != null) {
                        completedEnrollments = (int) course.getEnrolledStudents().stream()
                ...
                double avgQuizScore = quizAttemptRepository.findAll().stream()
                                .filter(attempt -> attempt.getQuiz() != null
                                                && attempt.getQuiz().getLesson() != null
                                                && attempt.getQuiz().getLesson().getCourse() != null)
                                .filter(attempt -> attempt.getQuiz().getLesson().getCourse().getId()
                                                .equals(course.getId()))
                ...
                if (course.getEnrolledStudents() != null) {
                        avgProgress = course.getEnrolledStudents().stream()
                                        .filter(student -> student != null)
                                        .mapToDouble(student -> calculateStudentProgress(student.getId(),
                                                        course.getId()))
                                        .average()
                                        .orElse(0.0);
                }
        ...
        public StudentEngagementMetrics getStudentEngagement(Long courseId) {
                ...
                if (course.getEnrolledStudents() != null) {
                        activeStudents = (int) course.getEnrolledStudents().stream()
                                        .filter(student -> student != null)
                                        .filter(student -> {
                                                ...
                                                return recentProgress.stream()
                                                                .anyMatch(p -> p != null && p.getLastWatchedAt() != null
                                                                                && p.getLastWatchedAt()
                                                                                                .isAfter(oneWeekAgo));
                                        })
                                        .count();
                ...
                if (course.getEnrolledStudents() != null) {
                        completedStudents = (int) course.getEnrolledStudents().stream()
                                        .filter(student -> student != null)
        ...
        private boolean hasRecentActivity(Long studentId, List<Course> courses, LocalDateTime since) {
                return courses.stream()
                                .flatMap(c -> progressRepository.findByStudentIdAndCourseId(studentId, c.getId())
                                                .stream())
        ...
        private double calculateCourseCompletionRate(Course course) {
                ...
                if (course.getEnrolledStudents() != null) {
                        completedCount = course.getEnrolledStudents().stream()
                                        .filter(student -> student != null)
                                        .filter(student -> isCourseCompleted(student.getId(), course.getId()))
                                        .count();
                }
